...
extension WxPayClient {
    ...
    // 统一下单
    func unifiedOrder(_ pramas: WxPayUnifiedOrderPramas, req: Request) throws -> EventLoopFuture<WxPayAppReqParams> {
        ...
        return try postWithParam(url: .unifiedOrder, params: pramas, req: req).flatMapThrowing { res -> WxPayUnifiedOrderResponse in
            ...
            let sign = try WxPaySign.sign(dic: MirrorExt.generateDic(model: signParam), mchSecret: self.mchSecret, signType: self.signType).uppercased()
            signParam.sign = sign
            return signParam
        }
    }
    // 订单查询
    func orderQuery(_ pramas: WxPayOrderQueryPramas, req: Request) throws -> EventLoopFuture<WxPayOrderQueryResp> {
        ...
        return try postWithParam(url: .orderQuery, params: pramas, req: req).flatMapThrowing({ (resp) -> WxPayOrderQueryResp in
            ...
            let sign2 = try WxPaySign.sign(dic: signDic, mchSecret: self.mchSecret, signType: self.signType)
    ...
    // 关闭订单
    func closeOrder(_ params: WxPayCloseOrderParams, req: Request) throws -> EventLoopFuture<WxPayCloseOrderResp> {
    ...
    // 退款
    func refundOrder(_ params: WxPayRefundOrderParams, req: Request) throws -> EventLoopFuture<WxPayRefundOrderResp> {
